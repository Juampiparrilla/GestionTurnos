...
- El formulario de "Crear producto" se puede **anclar** (ícono de pin en el título) para que quede abierto después de guardar, en vez de cerrarse, así se pueden cargar varios productos seguidos sin volver a abrirlo cada vez. Al crear uno siempre se confirma con un mensaje de éxito.
...
- **Actualizar costos**: buscá o filtrá cualquier grupo de productos (mismos filtros que Reportes) y aplicales de una un % de aumento o baja sobre el costo actual -- por ejemplo cuando un proveedor sube toda su lista un 15%. Las tres pistas de precio se recalculan solas con el % de ganancia que ya tenía cada una, sin tocar las fijadas manualmente.
