...
**En el celular** (pantallas de menos de 768 px) hay una barra fija abajo con cinco secciones — Inicio, Horarios, Productos, Caja y Más (ahí están Usuarios, solo para Super Administrador, y Cerrar sesión) — y la sección en la que estás parado se resalta en negrita con una marca terracota arriba. Dentro de Horarios (de un local), Productos y Caja, arriba de cada pantalla hay pestañas livianas (solo texto, con una raya abajo en la activa) para saltar directo a las demás pantallas de esa sección; el título de la pantalla se omite cuando la pestaña ya lo dice. En Productos las pestañas son Productos, Categorías, Marcas y Proveedores, y un menú ⋯ al lado de Crear producto lleva a Importar Excel, Actualizar costos, Reportes y Funcionalidades — así cada pantalla tiene un único botón fuerte y los filtros van en un botón con borde. **En la compu** se mantiene la vista de siempre: encabezado con los íconos de Usuarios/Horarios/Productos/Caja (el de la sección actual resaltado en negro y Salir a la derecha), accesos directos en Inicio y un menú con la flecha junto al título para saltar entre pantallas de una sección. **En el celular** la paleta es azul petróleo y arena (botones y acentos azul petróleo, fondos arena, y un toque terracota en la sección activa); **en la compu** sigue siendo blanco/gris/negro, sin colores de marca. En las dos, el color se reserva para dar significado puntual (ej. verde/rojo para Ingreso/Egreso en Caja, o distinguir secciones dentro de una misma pantalla).

Otros ajustes solo del celular: los botones, campos y selectores son más altos (40–44 px) para tocarlos sin errar; en los formularios de los paneles laterales el botón Guardar queda fijo abajo (arriba del teclado) sin tener que scrollear hasta el final, y la barra de abajo se esconde mientras se escribe; en el listado de Productos cada fila muestra el precio de venta, Editar/Desactivar/Borrar van en un menú ⋯ (para no tocar Borrar sin querer) y el buscador queda fijo arriba al scrollear (el listado arranca vacío, con una tarjeta "Sin filtros aplicados", hasta que se busca, se filtra o se toca "Ver todos los productos"; en la compu se ve completo siempre); y el botón Funcionalidades se reduce a un ícono "i" en la misma fila que el botón principal (Crear horario, Filtros del Resumen de Caja) para ganar lugar; y en todas las pantallas de Caja hay un botón flotante **Nuevo movimiento** que además recuerda el último local y turno usados en ese dispositivo, y el Resumen o los Movimientos que estén abiertos se actualizan solos al guardar.
...
| **Ayuda** | Ícono ⓘ a la derecha del botón principal. En las pantallas principales (Horarios, Resumen de Caja, Productos) abre "Funcionalidades"; en las demás (Categorías, Marcas, Proveedores, Etiquetas, Deudas, Actualizar costos, Reportes) muestra el texto que explica esa pantalla, que se oculta del cuerpo para ahorrar lugar. | El texto explicativo queda arriba de cada pantalla y hay un botón "Funcionalidades" de ancho completo. |
...
- Desde el Calendario, Configuración, Domingos o Feriados de un local, en el celular las pestañas de arriba (Calendario, Configuración —solo administradores—, Domingos, Feriados) llevan directo a cualquiera de esas pantallas de ese mismo local (para volver a la lista de locales, se toca Horarios en la barra de abajo); en la compu es la flecha de arriba, que abre un menú con esas mismas pantallas.
...
- Para saltar entre secciones: en el celular, pestañas arriba de cada pantalla (Productos, Categorías, Marcas, Proveedores; Costos y Reportes desde el menú ⋯) y la barra de abajo abre directamente el listado de Productos; en la compu, la flecha junto al título abre un menú para saltar directo a cualquier otra sección o al inicio.
...
- **Dashboard** (`/caja`): botón "Funcionalidades" con el detalle de todo lo de esta sección (igual que en Productos), un **Balance** grande arriba (el número que más importa de un vistazo) y, debajo, Ingresos/Egresos/Promedio diario más chicos como dato de apoyo, un gráfico de líneas con los ingresos por día (el eje X muestra hasta 5 fechas espaciadas parejo dentro del período, siempre incluyendo la primera y la última, para que no se amontonen ni se superpongan en el celular), y tres tarjetas colapsables (contraídas por defecto): "Venta por semana" (divide el mes en cuestión en 4 semanas fijas — 1-7, 8-14, 15-21 y 22-fin de mes — con una subtarjeta por semana mostrando Ingresos, Egresos y Promedio diario de esa semana; solo aparece cuando el período es "Este mes" o "Mes anterior", tomando el mes correspondiente en cada caso), "Mejor y peor día" e "Ingresos por turno". Todo filtrable por período (Hoy, Ayer, Últimos 7/30 días, Este mes — el que muestra por defecto —, Mes anterior o un rango a elección), local, turno y tipo, colapsado detrás de un botón "Filtros" con su "Limpiar filtros", como en Productos. Mientras carga, muestra un esqueleto gris con la forma de lo que va a aparecer en vez de un simple "Cargando...".
- **Movimientos** (`/caja/movimientos`, por defecto muestra el mes en curso): listado filtrable por fecha, local, turno, tipo y etiqueta, con un botón "+ Nuevo movimiento" arriba de los filtros que abre el alta en un panel lateral (se puede **anclar** para cargar varios seguidos, igual que "Crear producto" en Productos): Tipo (Ingreso ↑ / Egreso ↓) → Etiqueta (filtrada según el tipo elegido; se puede crear una nueva sin salir del formulario) → Importe → Fecha → Local → Turno (o "Sin turno") → Observación opcional. El campo Importe admite sumar varias ventas con `+` o restar con `-` (ej. `5000+3000+1500`), mostrando el total calculado en vivo. Cada fila muestra Tipo, Etiqueta e Importe, y se puede desplegar para ver fecha, local, turno, usuario y observación; los botones de Editar/Anular quedan separados del de "ver más" para no tocar el equivocado desde el celular. Un Administrador o Super Administrador puede editar cualquier campo de un movimiento (por si se cargó mal) o anularlo con un motivo opcional — un movimiento anulado no se borra, queda visible en el historial marcado como "Anulado" y deja de contar en los totales.
