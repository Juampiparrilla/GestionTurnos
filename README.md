...
- **Actualizar costos**: buscá o filtrá (mismos filtros que Reportes) para encontrar los productos y tildalos a mano -- de a uno, todos los visibles o el catálogo completo, la selección se mantiene entre búsquedas -- y aplicales de una un % de aumento (solo el número, ej. 15) o baja (con el signo "-" adelante, ej. -15) sobre el costo actual, por ejemplo cuando un proveedor sube toda su lista un 15%. Las tres pistas de precio se recalculan solas con el % de ganancia que ya tenía cada una, sin tocar las fijadas manualmente.
