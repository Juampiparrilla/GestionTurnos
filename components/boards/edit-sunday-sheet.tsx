...
export function EditSundaySheet({
  boardId,
  sunday,
  members,
  onOpenChange,
  ...
  const [date, setDate] = useState(sunday.sunday_date);
  const [userId, setUserId] = useState(sunday.user_id);
  ...
  async function handleSubmit() {
    ...
    const res = await fetch(`/api/boards/${boardId}/sundays/${sunday.id}`, {
      method: "PATCH",
  ...
  async function handleDelete() {
    ...
    if (!res.ok) {
      const data = await res.json();
      setError(data.error ?? "No se pudo eliminar el domingo.");
      setIsDeleting(false);
  ...
  return (
    <>
      ...
      <Sheet open onOpenChange={onOpenChange}>
        <SheetContent side="bottom" className="mx-auto w-full max-w-md">
          ...
          <div className="flex flex-col gap-4 px-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-sunday-user">Persona</Label>
              <Select value={userId} onValueChange={(v) => v && setUserId(v)}>
                <SelectTrigger id="edit-sunday-user">
                  <SelectValue>
                    {(value: string) =>
      ...
      <AlertDialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar este domingo?</AlertDialogTitle>
